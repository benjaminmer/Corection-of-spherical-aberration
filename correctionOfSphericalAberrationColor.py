...
for i, img_path in enumerate(image_path_list):
    if i == 0:
        images_color.append(cv2.imread(img_path, cv2.IMREAD_COLOR))
        img_for_alignment = cv2.imread(img_path, cv2.IMREAD_COLOR)
    else:
        color_image = cv2.imread(img_path, cv2.IMREAD_COLOR)
        b = align_image(img_for_alignment[:, :,0], color_image[:, :,0])
        g = align_image(img_for_alignment[:, :,1], color_image[:, :,1])
        r = align_image(img_for_alignment[:, :,2], color_image[:, :,2])
        images_color[0].append(b)
        images_color[1].append(g)
        images_color[2].append(r)

for channel in range(0, 3):
    for i, channel_image in enumerate(images_color[channel]):
        images_color[channel][i] = pad_image(channel_image)

color_image = []
for channel in range(0, 3):
    img_new_hard_polar_color = create_image_from_polar(create_new_image(images_color[channel], absolut_values), images_color[0][0])
    color_image.append(img_new_hard_polar_color)
    hard_edges = canny_sum(img_new_hard_polar_color)

color_image = cv2.merge(color_image)
cv2.imshow('Corrected Image', color_image)
cv2.imwrite('corrected_image_color.jpe', color_image)
print('edge count of corrected image', canny_sum(color_image))
